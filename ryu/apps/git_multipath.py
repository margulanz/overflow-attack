...
class Controller13(app_manager.RyuApp):
    ...
    def topology_discover(self, src, first_port, dst, last_port):
        ...
        self.logger.info("Possible paths: %s", paths)
	self.logger.info("Optimal Path with port: %s", path_with_port)
    ...
    def _packet_in_handler(self, ev):
        ...
        if eth.ethertype == ether_types.ETH_TYPE_IP and nw.proto == inet.IPPROTO_UDP:
            ...
            self.logger.info("IP Proto UDP from: %s to: %s",nw.src,nw.dst)
            ...
            self.logger.info(" IP Proto TCP from: %s to: %s", nw.src,nw.dst)
            ...
            self.logger.info(" IP Proto ICMP from: %s to: %s", nw.src,nw.dst)
            ...
            if arp_pkt.opcode == arp.ARP_REPLY:
                ...
                self.logger.info("ARP Reply from: %s to: %s H1: %s H2: %s",src_ip,dst_ip,h1,h2)
    ...
    def switch_enter_handler(self, ev):
        ...
        self.logger.info("Switch has been plugged in PID: %s",switch_dpid)
    ...
    def switch_leave_handler(self, ev):
        ...
        if switch in self.switches:
            try:
                ...
                self.logger.info("Switch has been already plugged off PID %s!",switch)
    ...
    def link_add_handler(self, ev):
        ...
        self.logger.info(
	    "Link between switches has been established, "
	    "SW1 DPID: %s:%s SW2 DPID: %s:%s",
	    ev.link.src.dpid,
	    ev.link.src.port_no,
	    ev.link.dst.dpid,
	    ev.link.dst.port_no
